...
for i=1:60
    ...
    for j=1:10
        %[TrainingTime, TestingTime, TrainAccuracy(j), TestAccuracy(j)] = elm(train_data, test_data, 1, 200, 'sin');
        [TrainingTime, TestingTime, TrainingAccuracy(j), TestingAccuracy(j)] = elm_kernel(train_data, test_data, 1, 100, 'lin_kernel', 1);
    end
